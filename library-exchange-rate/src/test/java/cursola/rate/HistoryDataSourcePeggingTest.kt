...
internal class HistoryDataSourcePeggingTest : AbstractDataSourceTest() {

    private lateinit var source: HistoryDataSource

    override fun prepare() {
        source = ExchangeRateModule.getInstance().historyValue(network, database)
    }
    ...
    private suspend fun expectValue(currency: String, value: Double) {
        val rates = source.get(Currency.getInstance(currency))
